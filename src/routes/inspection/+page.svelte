<script lang="ts">
	...
	import { ChevronLeft, ChevronRight, Image, ClipboardCheck } from '@lucide/svelte';
...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
	<div class="mb-6">
		<h1 class="flex items-center gap-2 text-2xl font-bold tracking-tight"><ClipboardCheck class="h-6 w-6" /> Inspeksi</h1>
	</div>
	...
	<form class="mb-6 rounded-xl border bg-card p-4 shadow-sm">
		<div class="flex flex-wrap items-end gap-3">
			<div class="w-40">
				<label for="inspection_date" class="mb-1.5 block text-xs font-medium text-muted-foreground">Tanggal</label>
				<select
					id="inspection_date"
				...
				>
					...
					{#each data.dates as date}
						<option value={date} selected={date === data.filters.inspection_date}>{date}</option>
					{/each}
				</select>
			</div>
			<div class="w-44">
				<label for="room_id" class="mb-1.5 block text-xs font-medium text-muted-foreground">Ruang</label>
				<select
					id="room_id"
					name="room_id"
					class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
				>
					...
					{#each data.rooms as room}
						<option value={room.id} selected={String(room.id) === data.filters.room_id}>{room.name}</option>
					{/each}
				</select>
			</div>
			<div class="w-36">
				<label for="kondisi" class="mb-1.5 block text-xs font-medium text-muted-foreground">Kondisi</label>
				<select
					id="kondisi"
					name="kondisi"
					class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
				>
					<option value="">Semua</option>
					<option value="Baik" selected={data.filters.kondisi === 'Baik'}>Baik</option>
					<option value="Rusak Ringan" selected={data.filters.kondisi === 'Rusak Ringan'}>Rusak Ringan</option>
					<option value="Rusak Berat" selected={data.filters.kondisi === 'Rusak Berat'}>Rusak Berat</option>
				</select>
			</div>
			<div class="min-w-[150px] flex-1">
				<label for="search" class="mb-1.5 block text-xs font-medium text-muted-foreground">Cari</label>
				<Input id="search" name="search" placeholder="Cari nama atau deskripsi..." value={data.filters.search} />
			</div>
			...
			<Table.Root>
				...
				<Table.Body>
					{#each data.items as item, i}
						<Table.Row>
							<Table.Cell class="text-center text-muted-foreground">{(data.page - 1) * 20 + i + 1}</Table.Cell>
							...
							<Table.Cell class="max-w-[150px] truncate">{item.remarks ?? '-'}</Table.Cell>
							<Table.Cell>
								<Button variant="ghost" size="icon" onclick={() => { imageUrl = item.imgUrl ? String(item.imgUrl) : null; imageDialogOpen = true; }}>
									<Image class="h-4 w-4" />
								</Button>
							</Table.Cell>
						</Table.Row>
					{:else}
						<Table.Row>
							<Table.Cell colspan={9} class="py-12 text-center text-muted-foreground">
	...
	<div class="mt-4 flex items-center justify-between">
		...
		{#if data.totalPages > 1}
			<div class="flex items-center gap-3">
				{#if data.page > 1}
					<Button variant="outline" size="default" href={buildUrl({ page: data.page - 1 })}>
						Sebelumnya
					</Button>
				{/if}
				<div class="flex items-center gap-1.5 text-sm">
					<span class="rounded-md bg-primary px-2.5 py-1 font-medium text-primary-foreground">{data.page}</span>
	...
	<Dialog.Root bind:open={imageDialogOpen}>
		<Dialog.Content class="sm:max-w-md">
			...
			<div class="flex flex-col items-center py-4">
				{#if imageUrl}
					<img src={imageUrl} alt="Inspection" class="max-h-64 rounded-lg border object-contain" />
				{:else}
